...
\begin{itemize}
  ...
  \item $\boldsymbol{\mu} \in \mathbb{R}^k$: the expected return
...
\begin{tikzpicture}[node distance=1.5cm, scale=0.9, every node/.style={scale=0.9}]
    \tikzstyle{block} = [rectangle, draw, fill=scrblue,
    ...
    \draw [arrow] (track.east) -- ++(2,0) |- node[near start, right, font=\footnotesize] {\textcolor{black}{Next Month}} (data.east);
\end{tikzpicture}
\end{center}
